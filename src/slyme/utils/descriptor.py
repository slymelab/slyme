...
from slyme.utils.abc.descriptor import DescriptorABC, DescriptorContainerABC
...
from slyme.utils.typing.extension import EmptyFlag
from .base import BaseObjectInit
from .base.collection import BaseList
from .typing.extension import STOP, MISSING

_T = TypeVar("_T")
_DescriptorT = TypeVar("_DescriptorT", bound=DescriptorABC)
DESCRIPTOR_PRIVATE_PREFIX = "smx_descriptor_"
...
class Descriptor(DescriptorABC[_T], BaseObjectInit, Generic[_T]):
...
class ReadonlyDescriptor(Descriptor[_T], Generic[_T]):
...
# NOTE: Avoid circular import error here.
...
class DescriptorContainer(
    Descriptor[_T],
    BaseList[_DescriptorT],
    DescriptorContainerABC[_DescriptorT, _T],
    Generic[_DescriptorT, _T],
):
    ...
    def __init__(
        ...
        *,
        allow_get_missing=False,
        list_like__: Union[Iterable[_T], EmptyFlag] = MISSING,
        **kwargs,
    ):
        super().__init__(
...
class CachedDescriptor(Descriptor[_T], Generic[_T]):
    ...
    def get_yield(self, instance, owner, value):
        if value is MISSING:
            ...
            with self.lock:
                current_value: _T = getattr(instance, self.private_name, MISSING)
...
class ReadonlyCachedDescriptor(
    DescriptorContainer[_DescriptorT, _T], Generic[_DescriptorT, _T]
):
    def __init__(self, /, factory: Callable[[Any], _T], **kwargs):
        super().__init__(
            ...
            list_like__=[
                ...
                ReadonlyDescriptor(missing_readonly=True),
