...
Auto owns its independent all-settled evaluation policy; Context owns nested cleanup groups, using sequential LIFO by default or explicit parallel disposal. Both report failures as exception groups without exposing partial successful results. Auto orders errors by input index; Context orders them by reverse registration order, independently of cleanup completion order. A returned exception remains ordinary data. Node and Wrapper wrap ordinary failures, including ordinary exception groups, in records whose `__cause__` retains the original error; existing Node records and non-`Exception` control failures propagate unchanged.
