...
Auto owns its independent all-settled evaluation policy; Context owns nested cleanup groups, using sequential LIFO by default or explicit batch disposal. Both report failures as exception groups without exposing partial successful results. Auto orders errors by input index; Context orders them by reverse registration order, independently of cleanup completion order. A returned exception remains ordinary data. Node and Wrapper wrap ordinary failures, including ordinary exception groups, in records whose `__cause__` retains the original error; existing Node records and non-`Exception` control failures propagate unchanged.
...
`Context(*, parent=None, scope=None, dispose_mode="sequential", facet_factory=None)` creates ownership, visibility, and an optional business object. Call `ctx.declare(schema_or_dict)` and then `ctx.update(values)` to initialize application data. Importing a Schema copies its current definitions, not its future changes. Context owns the parent/child tree and privately composes `Schema` for declarations, `ContextStore` for scoped data, and `Lifecycle` for effects and disposal state. Forks share Schema and Store, but each has its own Lifecycle bound to that Context. `dispose_mode="batch"` joins concurrent cleanup; every new Context defaults to sequential mode rather than inheriting its parent's mode. `ctx.children` snapshots direct children in creation order, retaining children until their cleanup completes. Use `ctx.resolve(path)`, `ctx.resolve_entry(path)`, and `ctx.entries` to inspect application declarations; `ctx.declare()` also owns their cleanup. Normal access checks the calling Context's lifecycle, while exact registration withdrawal and Scope release remain available during cleanup.

Roots (`parent=None`) require sequential disposal so framework defaults outlive user cleanup. Use a child Context with `dispose_mode="batch"` for concurrent cleanup; a root requesting batch mode raises `ValueError` before initialization.

`Context[A].facet` is an ordinary read-only attribute of type `A`; bare `Context` defaults to `Context[Any]`. `Context()`, `fork()`, and `derive()` accept `facet_factory(ctx)` and store its result without automatically calling its methods. Factories run synchronously after initialization, including root defaults and any `derive()` bindings. A child never inherits its parent's facet; without a factory its value is `None`, and `Context[None]` is optional when that type matters. A facet remains available after disposal. Prefer registering effects after construction: factory failures use synchronous disposal for rollback and cannot wait for asynchronous setup or cleanup. See [Context facets](docs/src/guide/essentials/lifecycle.md#context-facets) for typed business objects and activation lifetimes.
