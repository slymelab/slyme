...
For mixed execution, write an ordinary generator and pass it to `run()` from `slyme.utils.continuation`. Each `value = yield operation()` accepts an immediate or asynchronous result. The driver runs synchronously until an awaitable is yielded, then returns an unscheduled awaitable remainder. Loops, branches, and `try/except/finally` stay in the generator. Use `await await_result(run(generator))` when either return mode is possible. The driver does not schedule tasks, aggregate errors, or own resource lifetimes.
