...
`Continuation.call(operation).then(transform).unwrap()` composes immediate and asynchronous results in an ordinary function. Chains are mutable and single-use; constructing them runs no callbacks. `unwrap()` runs the synchronous prefix and returns a value or awaitable, while `aunwrap()` always returns an awaitable with the same immediate prefix. The chain itself is not awaitable; nested chains must be explicitly unwrapped. `Continuation.sequential(values, call)` builds fail-fast ordered calls and returns their values; `continue_on_error=True` attempts remaining calls before reporting failure. `Continuation.batch(values, call)` attempts all inputs, runs asynchronous calls concurrently when awaited, and returns an input-ordered list or raises `BatchError`. Both support `then()` and `catch()` before execution. `BatchError.results` stores input-ordered `Result` records: `value` holds a successful return, while `error` holds a raised exception. Successful partial results remain available after failure; nested batches keep local indices.

Auto uses batches both across independent evaluator groups and within Ref and Node evaluation. A Ref failure does not prevent Node evaluation. After the first awaitable child or cleanup, pending and remaining children execute concurrently when awaited. Every sibling is attempted; child failure or cancellation never cancels siblings. All errors are retained in nested `BatchError` objects after the batches settle; final child cleanup errors form the Node batch's cause. Cancelling the outer evaluation Task follows asyncio propagation; cancellation alone propagates directly, while other failures remain observable through `BatchError` with cancellation as a cause. Application code owns timeouts and abort policies. Each child has an owned Context and distinct Scope; successful children dispose immediately, and final cleanup completes before error or cancellation propagation, including repeated cancellation. Returned values must not depend on child-owned resources. `sequential_exec(ctx, children)` instead stops at failure and shares the supplied Context; `sequential(nodes=children)` builds that ordering as a Node.
