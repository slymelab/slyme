...
`Continuation.call(operation).then(transform).unwrap()` composes immediate and asynchronous results in an ordinary function. Chains are mutable and single-use; constructing them runs no callbacks. `unwrap()` runs the synchronous prefix and returns a value or awaitable, while `aunwrap()` always returns an awaitable with the same immediate prefix. The chain itself is not awaitable; nested chains must be explicitly unwrapped. `Continuation.sequential(values, call)` builds fail-fast ordered calls, discarding results. `Continuation.batch(values, call)` attempts all inputs, runs asynchronous calls concurrently when awaited, and returns an input-ordered list or raises `BatchError`. Both support `then()` and `catch()` before execution. `BatchError.errors` maps zero-based input indices to exceptions; nested batches keep local indices.
