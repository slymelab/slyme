...
- `Context.effect()` owns immediate or awaitable setup and cleanup. Use `resolve` from `slyme.utils.awaitable` to await either form. Setup and cleanup cannot dispose their owner or an ancestor. `dispose()` runs synchronous cleanup immediately and returns an awaitable if unfinished; await it to finish recursive LIFO cleanup. Repeated disposal shares its completion and reproduces terminal failures.
