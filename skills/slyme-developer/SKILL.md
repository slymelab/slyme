...
- `Compose` stores ordered values resolved through Scope C3 lookup. `compose.derive(parents=..., binding=...)` creates a configured Scope; `Compose.derive_many(parents=..., bindings=...)` configures several Composes on one new Scope. Both require parents and create no lifecycle. Register with `ctx.effect(lambda: compose.add(scope, value))` when the Context should own cleanup; otherwise manage the disposer returned by `compose.add()` explicitly.
