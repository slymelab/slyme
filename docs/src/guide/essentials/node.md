...
Use `get(name)` to read a binding, `set(name, value)` to store any binding, and `delete(name)` to remove one. Missing keys raise `KeyError` unless `get(name, default)` supplies a fallback, which may be any value including `None`. The fallback is returned without saving a binding; an existing value, including `None`, takes precedence. These operations never read function defaults. The live, read-only `params` mapping contains only saved bindings. `node(ctx, **kwargs)` and `node.acall(ctx, **kwargs)` shallowly override those bindings for one call before Auto evaluation, without updating `params` or merging nested containers. Parameter names may overlap framework attributes because parameters are not projected as object attributes.
