...
Auto attempts every sibling and every evaluator group, even after a synchronous failure. Evaluator groups are independent and may execute concurrently; a Ref lookup failure does not prevent Node evaluation. A child failure or cancellation does not cancel siblings: evaluation waits for all of them, like `asyncio.gather(..., return_exceptions=True)`. Failures are raised together as `BatchError` from `slyme.utils.continuation`. Its `errors` map is indexed by evaluator-group order of first appearance; built-in evaluators contain another `BatchError` indexed by the Ref or Node's position within that group. Final child cleanup failures form a separate `BatchError` attached as the Node batch's cause. Repeated disposal does not duplicate the same retained cleanup failure. Node and Wrapper calls preserve these aggregates. Exception objects returned as ordinary values remain data. A child that never finishes keeps evaluation pending; application code owns timeouts and abort policies.

Cancelling the outer evaluation Task propagates to its pending children through asyncio. Auto waits for those tasks to exit and for their Context cleanup to finish, including after repeated cancellation. Cancellation propagates directly when no other failure occurs; otherwise a `BatchError` preserves the failures and the cancellation is retained as a cause. A synchronous child runs inline on the event-loop thread and cannot be interrupted while executing. Task cancellation does not guarantee that underlying network, thread, or process work has stopped; application adapters own that behavior.
