...
- Each Node and Wrapper call freezes its current ordinary parameter containers into a local immutable snapshot.
...
For concurrency, explicitly copy Context when branches require isolated state. Call-local parameter snapshots prevent later graph changes from altering an invocation already in progress, but they do not make arbitrary shared Python objects thread-safe.
