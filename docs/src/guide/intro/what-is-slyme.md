...
- **Explicit Copy Boundaries**: Context data and Node parameters remain mutable for efficient long-lived workflows. Callers use `.clone()` when a branch requires an independent Node/PyTree or ContextData structure.
...
[Context](/guide/essentials/context) is Slyme's hierarchical runtime data store. Its outer attributes are stable while its data is mutable in place. [Ref](/guide/essentials/context#ref) identifies paths such as `a.b`; call `context.clone()` when a branch requires an independent ContextData hierarchy.
...
### Live Graph and Explicit Clones
...
- **When isolation is needed**: `.clone()` explicitly rebuilds registered Node/PyTree or ContextData structure while preserving unregistered leaves.
