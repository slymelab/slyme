...
Slyme 提供了一个面向 Coding Agent 的 Agent Skill，用于帮助代理在其他项目中基于 Slyme 开发应用。它包含 `@node`、`@wrapper`、图的组装、`Context`、`Ref`、`Auto`、Tree 和命令行参数等推荐用法。
...
读取记录，通过 node 标准化，并将结果写入 Context。
```
