...
- **显式复制边界**：Context 数据与 Node 参数保持可变，以支持高效的长生命周期工作流。分支需要独立 Node/PyTree 或 ContextData 结构时，由调用方显式调用 `.clone()`。
...
[Context](/zh/guide/essentials/context) 是 Slyme 的层次化运行时数据存储。它的外层属性保持稳定，内部数据原地可变。[Ref](/zh/guide/essentials/context#ref) 用于标识 `a.b` 等路径；分支需要独立 ContextData 层次时调用 `context.clone()`。
...
- **需要隔离时**：`.clone()` 显式重建已注册的 Node/PyTree 或 ContextData 结构，同时保留未注册叶子。
