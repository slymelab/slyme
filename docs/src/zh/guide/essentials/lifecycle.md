...
即使 effect 清理失败，仍会执行收尾。如果收尾也失败，则抛出收尾异常，清理异常组保留在其 `__context__` 中，遵循 Python 的 `finally` 语义，不改写 `__cause__`。失败时 Lifecycle 仍进入 `DISPOSED`；重复释放会观察到同一个最终异常。
...
Context 拥有子 Context，以及通过 `effect()`、`register()` 和 `declare()` 注册的 cleanup。其 `dispose_mode` 决定直接归属项采用串行还是批量清理。`dispose()` 同步完成时返回 `None`，遇到异步清理则返回 awaitable；两者均可使用 `await await_result(ctx.dispose())` 完成。Context 不会自动拥有使用它的任意 task，应用应先停止并等待这些 task，再释放 Context。
...
这里 plugin_a 与 plugin_b 成批清理，异步部分并发等待，各自内部保持 LIFO。根声明和框架默认配置保留到两者结束。继续 fork 可以任意嵌套清理策略，不增加 Scope 继承层。操作归属于调用它的 Context，不存在隐式的当前分组或 disposer 转移。

同一批中的兄弟项必须允许清理重叠。共享依赖应先登记到串行父级，再创建消费者，或者显式建立等待关系。清理期间允许读取，并不阻止其他并发 effect 撤销字段。Context 不会根据数据可见性推导插件卸载顺序。
