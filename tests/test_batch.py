...
async def test_batch_only_groups_raised_errors_not_returned_exceptions(
    ...
    with pytest.raises(BaseExceptionGroup) as caught:
        await await_result(_run_batch(range(3), acall if asynchronous else call))
    assert caught.value.exceptions == (error,)
    assert caught.value.message == "Evaluation failed at input indices [1]"
    assert await await_result(_run_batch([0, 2], acall if asynchronous else call)) == [
        error,
        None,
    ]
...
def test_sync_evaluation_batch_runs_immediately(values) -> None:
    ...
    assert _run_batch(values, call) == [2, 4]
    assert visited == [1, 2]
    assert _run_batch([], call) == []
...
async def test_batch_orders_errors_by_input_not_completion() -> None:
    ...
    with pytest.raises(BaseExceptionGroup) as caught:
        await await_result(_run_batch(range(3), call))
...
async def test_batch_waits_for_later_calls_after_sync_failure() -> None:
    ...
    with pytest.raises(BaseExceptionGroup) as caught:
        await await_result(_run_batch([0, 1], call))
...
async def test_batch_keeps_exceptions_as_data_and_nested_error_indices() -> None:
    ...
    assert await await_result(_run_batch([1, 2], call)) == [data, data]
...
async def test_batch_schedules_only_asynchronous_items() -> None:
    ...
    before = asyncio.all_tasks()
    pending = _run_batch(range(5), call)
    assert asyncio.all_tasks() == before
...
async def test_caller_cancellation_waits_for_submitted_calls(suppress: bool) -> None:
    ...
    task = asyncio.create_task(await_result(_run_batch([1, 2], call)))
    await started.wait()
    task.cancel()
...
async def test_caller_cancellation_propagates_without_aggregating_partial_failures() -> (
    ...
    task = asyncio.create_task(await_result(_run_batch([0, 1], call)))
    await started.wait()
    task.cancel()
    with pytest.raises(asyncio.CancelledError) as caught:
        await task
